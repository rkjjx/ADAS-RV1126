...
if __name__ == '__main__': 
    img = cv2.imread('/home/alientek/ADAS/model_trans/dataset/face_dataset/face_4.jpg') 
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) 
    img = cv2.resize(img, (112,112), interpolation=cv2.INTER_LINEAR)
    # 1. RKNN 推理
    rknn_outs = inference_with_rknn(target='rv1126', device_id='20230331RV1126FD2G0033', inputs=[img]) 
    
    # 2. ONNX 推理 (替代原来的 Torch 推理)
    onnx_outs = inference_with_onnx(img=img, onnx_path=ONNX_PATH) 
    
    # 3. 对比相似度
    # rknn_outs[0] 是 numpy 数组，onnx_outs 也是 numpy 数组，直接计算
    cos_dis = compute_cos_sim(rknn_outs[0], onnx_outs[0]) 
